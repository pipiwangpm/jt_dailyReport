...
先确认这个工具的产品形态：每天通过当前对话或钉钉机器人记录动作和待办，网页只负责按日历审查记录、修改记录、查看历史报告并发送日报。当前阶段采用移动优先原型，不处理视觉精修、数据库或 AI 接入。
...
用途：按天查看从当前对话或钉钉归集来的记录，优先适配手机使用。

核心模块：
- 月历：用样式区分当天是否有记录或报告。
- 当天记录：展示所选日期的完成项和待办项。
- 修改记录：允许修改标题、项目、日期、状态和是否纳入日报。

空状态：
- 所选日期无记录时，显示“当天暂无记录”。

主要跳转：
- 底部导航切换到日报日历，查看或生成所选日期日报。

### 智能归集

用途：承接来自当前对话或钉钉机器人的工作记录，不再放在网页主界面。
...
用途：按日期查看历史日报、生成新日报，并在人工审查后发送到钉钉。

核心模块：
- 月历：用样式区分当天是否有记录、是否有历史报告。
- 历史报告：查看所选日期已生成的日报草稿。
